...
/*
  ...
  service cloud.firestore {
    match /databases/{database}/documents {
      ...
      match /product_reviews/{productId}/reviews/{reviewId} {
        allow read: if true;
        allow create: if request.auth != null;
      }
...
(function () {
  ...
  waitForSDK(function () {
    try {
      ...
      const ALLOW_ANONYMOUS_ONLINE_CART = false;
      let anonymousBlocked = false;

      async function ensureUser(options = {}) {
        const allowAnonymous = options.allowAnonymous ?? ALLOW_ANONYMOUS_ONLINE_CART;
        ...
        try {
          ...
          console.warn('Login anônimo indisponível no Firebase. O cliente precisa entrar na conta para salvar dados online.', err);
          return null;
        }
      }
      ...
      window.FB = {
        ...
        async getCart() {
          try {
            ...
            console.error('getCart erro:', err);
            return [];
          }
        },
        ...
        async saveCart(items) {
          try {
            const user = await ensureUser();
            if (!user) return { ok: false, msg: 'Entre na sua conta para salvar o carrinho online.' };
            ...
            console.error('saveCart erro:', err);
            return { ok: false };
          }
        },
        ...
        async listenCart(callback) {
          try {
            ...
            return db.collection('carts').doc(user.uid).onSnapshot(
              snap => {
                const data = snap.exists ? snap.data() : {};
                callback(Array.isArray(data.items) ? data.items : []);
              },
              err => {
                console.error('listenCart erro:', err);
                callback([]);
              }
            );
          } catch (err) {
            console.error('listenCart init erro:', err);
            callback([]);
            return () => {};
          }
        },
        ...
        async getWishlist() {
          try {
            ...
            console.error('getWishlist erro:', err);
            return [];
          }
        },

        async saveWishlist(ids) {
          try {
            const user = await ensureUser();
            if (!user) return { ok: false, msg: 'Entre na sua conta para salvar a lista de desejos online.' };
            await db.collection('users').doc(user.uid).set({
              wishlist: Array.isArray(ids) ? ids.map(String) : [],
              updatedAt: firebase.firestore.FieldValue.serverTimestamp(),
            }, { merge: true });
            return { ok: true };
          } catch (err) {
            console.error('saveWishlist erro:', err);
            return { ok: false };
          }
        },
        ...
        async getProductReviews(productId) {
          try {
            const snap = await db.collection('product_reviews').doc(String(productId))
              .collection('reviews').orderBy('createdAt', 'desc').get();
            return snap.docs.map(d => ({ ...d.data(), id: d.id }));
          } catch (err) {
            console.warn('Avaliações indisponíveis. Publique firestore.rules para liberar leitura.');
            return [];
          }
        },
        ...
        async saveProductReview(productId, review) {
          try {
            const user = await ensureUser();
            if (!user) return { ok: false, msg: 'Entre na sua conta para publicar avaliações.' };
            await db.collection('product_reviews').doc(String(productId)).collection('reviews').add({
              ...review,
              userId: user.uid,
              userEmail: user.email || '',
              createdAt: firebase.firestore.FieldValue.serverTimestamp(),
            ...
            console.error('saveProductReview erro:', err);
            return { ok: false };
          }
        },
        ...
        async saveOrder(order) {
          try {
            const user = auth.currentUser || await ensureUser();
            if (!user) return { ok: false, msg: 'Entre na sua conta para finalizar o pedido online.' };
