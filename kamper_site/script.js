/* ============================================================
   ...
   Carrinho, conta, pedidos e catálogo vivem no Firebase.
   ============================================================ */
'use strict';
...
const Cart = {
  _unsubscribe: null,
  _ready: false,
  get()      { return memoryState.cart.slice(); },
  set(c) {
    memoryState.cart = Array.isArray(c) ? c.map(item => ({ id: String(item.id), qty: Number(item.qty) || 1 })) : [];
    this._ready = true;
    updateCartCount();
    window.dispatchEvent(new CustomEvent('cart-updated'));
  },
  async startSync(force = false) {
    if (!window.FB?.getCart) return;
    if (this._unsubscribe && !force) return;
    if (this._unsubscribe) {
      this._unsubscribe();
      this._unsubscribe = null;
    }
    this.set(await window.FB.getCart());
    if (window.FB.listenCart) {
      this._unsubscribe = await window.FB.listenCart(items => this.set(items));
    }
  },
  async save(c) {
    const nextCart = Array.isArray(c) ? c.map(item => ({ id: String(item.id), qty: Number(item.qty) || 1 })) : [];
    if (!window.FB?.saveCart) {
      showToast('Firebase indisponível. Entre novamente para usar o carrinho online.', 'error');
      return false;
    }
    const result = await window.FB.saveCart(nextCart);
    if (!result?.ok) {
      showToast(result?.msg || 'Não foi possível salvar o carrinho online.', 'warning');
      return false;
    }
    this.set(nextCart);
    return true;
  ...
  async add(productId) {
    ...
    if (!prod)            { showToast('Produto não encontrado.'); return false; }
    if (prod.stock === 0) { showToast(`"${prod.name}" está indisponível.`); return false; }
    const cart = this.get();
    const item = cart.find(i => sameId(i.id, id));
    if (item && item.qty >= prod.stock) {
      showToast(`Apenas ${prod.stock} unidade(s) disponíveis.`); return false;
    }
    if (item) item.qty++; else cart.push({ id, qty: 1 });
    const saved = await this.save(cart);
    if (!saved) return false;
    showToast(`"${prod.name}" adicionado ao carrinho! 🛒`);
    return true;
  },
  async remove(productId) {
    await this.save(this.get().filter(i => !sameId(i.id, productId)));
  },
  async setQty(productId, qty) {
    ...
    const saved = await this.save(cart);
    if (saved && qty > prod.stock) showToast(`Apenas ${prod.stock} unidade(s) disponíveis.`);
  },
  async clear() { await this.save([]); },
};
window.Cart = Cart;
...
const Auth = {
  ...
  async register(name, email, password) {
    ...
    if (r.ok) {
      memoryState.session = { name, email, uid: r.user.uid };
      if (previousCart.length) await window.FB.saveCart(previousCart);
      await Cart.startSync(true);
    }
    return r;
  },
  ...
  updateSessionProfile(patch) {
    if (memoryState.session) memoryState.session = { ...memoryState.session, ...patch };
  },
...
function initCheckout() {
  ...
  if (itemsEl) {
    if (items.length && !prods.length) {
      itemsEl.innerHTML = '<div style="padding:24px 0;color:var(--gray-400);font-size:.9rem">Carregando produtos...</div>';
    } else {
  ...
  if (!initCheckout._bound) {
    window.addEventListener('products-updated', initCheckout);
    window.addEventListener('cart-updated', initCheckout);
    initCheckout._bound = true;
  }
}

/* ── Boot ────────────────────────────────────────────────────── */
document.addEventListener('DOMContentLoaded', () => {
  /* Inicia páginas imediatamente; produtos chegam pelo Firebase. */
  initHeader();
  initHome();
  initProducts();
  initCart();
